...
endpoint="https://scannedpdf.cognitiveservices.azure.com/"
key="7kYA05kdWVpCxRNcZUlfgjIXue3tY3wQqINfJJj5I7lrZklS9ACkJQQJ99BEACYeBjFXJ3w3AAALACOGfpHj"

def analyze_invoice():
    # Local file path to the invoice
    invoice_path = "scanned/invoice scan_compressed.pdf"
    
    # Check if file exists
    if not os.path.exists(invoice_path):
        print(f"Error: File not found at {invoice_path}")
        return
    ...
    # Open the file and analyze it directly
    with open(invoice_path, "rb") as f:
        invoice_data = f.read()
        poller = document_intelligence_client.begin_analyze_document(
            "prebuilt-invoice", invoice_data
        )
            ...
            if items and items.get("valueArray"):
                for idx, item in enumerate(items.get("valueArray")):
                    ...
                    if item.get("valueObject"):
                        ...
                        if unit_price:
                            unit_price_code = ""
                            if unit_price.get("valueCurrency") and unit_price.get("valueCurrency").get("currencyCode"):
                                unit_price_code = unit_price.get("valueCurrency").get("currencyCode")
                            print(
